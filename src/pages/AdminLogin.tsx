...
import { sanitizeInput, rateLimiter, logSecurityEvent } from "@/utils/security";
import { SecureAdminRegistration } from "@/components/admin/SecureAdminRegistration";
...
export default function AdminLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Rate limiting
    const clientId = `admin_login_${sanitizedUsername}`;
    if (!rateLimiter.checkAttempt(clientId, 5, 15 * 60 * 1000)) {
    ...
    try {
      ...
      // Construct admin email from username
      const email = sanitizedUsername.includes('@') ? sanitizedUsername : `${sanitizedUsername.toLowerCase()}@escortsreloaded.com`;
      ...
      logSecurityEvent('admin_login_success', { email, userId: data.user.id });
          ...
          <form onSubmit={handleLogin} className="mt-8 space-y-5">
            ...
            <div className="text-center">
              <Button
                type="button"
                variant="ghost"
                className="text-[#9b87f5] hover:text-purple-400 text-sm"
